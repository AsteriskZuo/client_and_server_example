...
//    LoadStringW(hInstance, IDC_WINDOWSSERVER, szWindowClass, MAX_LOADSTRING);
...
//    }
//
//    HACCEL hAccelTable = LoadAccelerators(hInstance, MAKEINTRESOURCE(IDC_WINDOWSSERVER));
//
//    MSG msg;
//
//    // 主消息循环:
...
//    wcex.hIcon          = LoadIcon(hInstance, MAKEINTRESOURCE(IDI_WINDOWSSERVER));
//    wcex.hCursor        = LoadCursor(nullptr, IDC_ARROW);
//    wcex.hbrBackground  = (HBRUSH)(COLOR_WINDOW+1);
//    wcex.lpszMenuName   = MAKEINTRESOURCEW(IDC_WINDOWSSERVER);
...
int main(int argc, char **argv)
{
	...
	//char* argv2 = GetCommandLineA();
	//int argc2 = 0;
	//LPWSTR* argv3 = CommandLineToArgvW(GetCommandLineW(), &argc2);

	//int argc = argc2;
	//char **argv = &argv2;

	fprintf(stdout, "Start up windows_server progma %d\n", argc);
...
DWORD WINAPI ServerThread(LPVOID lpParam)
{
	...
	for (;;)
	{
		...
		if (socket_type == SOCK_STREAM)
		{
			...
			{
				//
				// If we have a client connection recv and send until is_done
				//
